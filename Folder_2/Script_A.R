...
##### DON'T DO THIS FOR CALCULATION OF FIMO ROC
...
# obtain FIMO ranks for unique genes
cutsites = cutsites[order(cutsites$fimo_rank_normalised),]
...
###### 
## GENERATE input for Fig 3c and Fig 3d for Venn diagrams

# extract the last known cutsite P4P1prime based on each criteria
# based on fimo rank (at cutsite level)
cutsites = cutsites[order(cutsites$score, decreasing = TRUE),]
cutsites_subset_P4P1prime_fimo_rank = cutsites
#cutsites_subset_P4P1prime_fimo_rank$gene_rank = rank(cutsites$score, ties.method = "min")
max_fimo_rank_P4P1prime = max(which(cutsites_subset_P4P1prime_fimo_rank$literature_cleaved==1 &
                                      !is.na(cutsites_subset_P4P1prime_fimo_rank$exact_match_shell2_score)))
# obtain fimo score at this criteria
temp_fimo = cutsites_subset_P4P1prime_fimo_rank$score[max_fimo_rank_P4P1prime]
# subset those with sequence rank equal or better
max_fimo_rank_P4P1prime = max(which(cutsites_subset_P4P1prime_fimo_rank$score == temp_fimo))
fimo_P4P1prime = cutsites_subset_P4P1prime_fimo_rank$Matched.sequence[1:max_fimo_rank_P4P1prime]
fimo_P4P1prime_unique = unique(fimo_P4P1prime)
# remove known cleavage sites to yield only CANDIDATE cutsites
fimo_P4P1prime_unique = fimo_P4P1prime_unique[! fimo_P4P1prime_unique %in% cutsite_coop$P4_P1prime]
write.csv(fimo_P4P1prime_unique, "unique_P4P1prime_fimo.csv")

# get a single rank for each unique protein
cutsites = cutsites[match(unique(cutsites$Gene_name), cutsites$Gene_name),]
# based on fimo rank (at GENE / PROTEIN level)
cutsites = cutsites[order(cutsites$score, decreasing = TRUE),]
cutsites_subset_P4P1prime_fimo_rank = cutsites
#cutsites_subset_P4P1prime_fimo_rank$gene_rank = rank(cutsites$score, ties.method = "min")
max_fimo_rank_P4P1prime = max(which(cutsites_subset_P4P1prime_fimo_rank$literature_cleaved==1 &
...
# subset those with sequence rank equal or better
max_fimo_rank_P4P1prime = max(which(cutsites_subset_P4P1prime_fimo_rank$score == temp_fimo))
fimo_P4P1prime = cutsites_subset_P4P1prime_fimo_rank$Entry.name[1:max_fimo_rank_P4P1prime]
fimo_P4P1prime_unique = unique(fimo_P4P1prime)
# remove known cleavage sites to yield only CANDIDATE cutsites
fimo_P4P1prime_unique = fimo_P4P1prime_unique[! fimo_P4P1prime_unique %in% cutsite_coop$P4_P1prime]
write.csv(fimo_P4P1prime_unique, "unique_proteins_fimo.csv")
